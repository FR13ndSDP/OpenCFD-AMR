...
const int NSPECS = 7, NREACS = 16;

// kg/mol
static const double mw[NSPECS] = {1.008 * 1e-3,   /*H */
                                  15.999 * 1e-3,   /*O */
                                  18.015 * 1e-3,   /*H2O */
                                  17.007 * 1e-3,   /*OH */
                                  31.998 * 1e-3,   /*O2 */
...
// K
static const double HP[NSPECS] = {H_lo[5], O_lo[5], H2O_lo[5], OH_lo[5], O2_lo[5], H2_lo[5], N2_lo[5]};

// used to get initial T
static const double A0[NSPECS] = {H_lo[0]-1.0, O_lo[0]-1.0, H2O_lo[0]-1.0, OH_lo[0]-1.0, O2_lo[0]-1.0, H2_lo[0]-1.0, N2_lo[0]-1.0};

// Pa
static const double Patm = 101325.0, Ru = 8.31446261815324;

void CKPY(const double *rhoi, double T, double &P) {
...
void CKRHOY(double P, double T, const double *y, double &rho) {
  double YOW = 0;
  double tmp[NSPECS];
...
void CKYTX(const double *y, double *x) {
  double YOW = 0;
  double tmp[NSPECS];
...
void CKXTY(const double *x, double *y) {
...
// gi/T, gi = g/Ri
void gibbs(double *species, const double *tc) {
  /*temperature */
  double T = tc[1];
  double invT = 1 / T;
  ...
  if (T < 1000) {
    /*species 0: H */
    species[0] = H_lo[0] * (1.0 - tc[0]) - H_lo[1] / 2.0 * tc[1] -
                 H_lo[2] / 6.0 * tc[2] - H_lo[3] / 12.0 * tc[3] -
                 H_lo[4] / 20.0 * tc[4] + H_lo[5] * invT - H_lo[6];
    /*species 1: O */
    species[1] = O_lo[0] * (1.0 - tc[0]) - O_lo[1] / 2.0 * tc[1] -
    ...
    /*species 3: OH */
    species[3] = OH_hi[0] * (1.0 - tc[0]) - OH_hi[1] / 2.0 * tc[1] -
                 OH_hi[2] / 6.0 * tc[2] - OH_hi[3] / 12.0 * tc[3] -
                 OH_hi[4] / 20.0 * tc[4] + OH_hi[5] * invT - OH_hi[6];
    /*species 4: O2 */
    species[4] = O2_hi[0] * (1.0 - tc[0]) - O2_hi[1] / 2.0 * tc[1] -
                 O2_hi[2] / 6.0 * tc[2] - O2_hi[3] / 12.0 * tc[3] -
                 O2_hi[4] / 20.0 * tc[4] + O2_hi[5] * invT - O2_hi[6];
    /*species 5: H2 */
    species[5] = H2_hi[0] * (1.0 - tc[0]) - H2_hi[1] / 2.0 * tc[1] -
                 H2_hi[2] / 6.0 * tc[2] - H2_hi[3] / 12.0 * tc[3] -
                 H2_hi[4] / 20.0 * tc[4] + H2_hi[5] * invT - H2_hi[6];
    /*species 6: N2 */
    species[6] = N2_hi[0] * (1.0 - tc[0]) - N2_hi[1] / 2.0 * tc[1] -
                 N2_hi[2] / 6.0 * tc[2] - N2_hi[3] / 12.0 * tc[3] -
                 N2_hi[4] / 20.0 * tc[4] + N2_hi[5] * invT - N2_hi[6];
  }
}

void vproductionRate(double *wdot, double (&Arate)[NSPECS][NSPECS], const double *sc,
                     double T) {
  double k_f_s[NREACS], Kc_s[NREACS], mixture, gi_T[NSPECS];
  double tc[5], invT;
  ...
  k_f_s[0] = 5.5e+18 * exp(-1.0 * tc[0] - 1.033e+05 * invT);
  k_f_s[1] = 1.8e+18 * exp(-1.0 * tc[0]);
  k_f_s[2] = 7.2e+18 * exp(-1.0 * tc[0] - 1.1791e+05 * invT);
  k_f_s[3] = 4.0e+17 * exp(-1.0 * tc[0]);
  k_f_s[4] = 5.2e+21 * exp(-1.5 * tc[0] - 1.18e+05 * invT);
  k_f_s[5] = 4.4e+20 * exp(-1.5 * tc[0]);
  k_f_s[6] = 8.5e+18 * exp(-1.0 * tc[0] - 1.01e+05 * invT);
  k_f_s[7] = 7.1e+18 * exp(-1.0 * tc[0]);
  k_f_s[8] = 5.8e+13 * exp(-1.8e+04 * invT);
  k_f_s[9] = 5.3e+12 * exp(-1000.0 * invT);
  k_f_s[10] = 8.4e+13 * exp(2.01e+04 * invT);
  k_f_s[11] = 2.0e+13 * exp(-5166.0 * invT);
  k_f_s[12] = 2.2e+14 * exp(-1.68e+04 * invT);
  k_f_s[13] = 1.5e+13;
  k_f_s[14] = 7.5e+13 * exp(-1.11e+04 * invT);
  k_f_s[15] = 3.0e+13 * exp(-8800.0 * invT);
  ...
  Kc_s[0] = 1.0 / RsT * exp(gi_T[5] - (gi_T[0] + gi_T[0]));
  Kc_s[1] = RsT * exp((gi_T[0] + gi_T[0]) - gi_T[5]);
  Kc_s[2] = 1.0 / RsT * exp(gi_T[4] - (gi_T[1] + gi_T[1]));
  Kc_s[3] = RsT * exp((gi_T[1] + gi_T[1]) - gi_T[4]);
  Kc_s[4] = 1.0 / RsT * exp((gi_T[2]) - (gi_T[0] + gi_T[3]));
  Kc_s[5] = RsT * exp((gi_T[0] + gi_T[3]) - (gi_T[2]));
  Kc_s[6] = 1.0 / RsT * exp(gi_T[3] - (gi_T[0] + gi_T[1]));
  Kc_s[7] = RsT * exp((gi_T[0] + gi_T[1]) - (gi_T[3]));
  Kc_s[8] = exp((gi_T[2] + gi_T[1]) - (gi_T[3] + gi_T[3]));
  Kc_s[9] = exp((gi_T[3] + gi_T[3]) - (gi_T[2] + gi_T[1]));
  Kc_s[10] = exp((gi_T[2] + gi_T[0]) - (gi_T[3] + gi_T[5]));
  Kc_s[11] = exp((gi_T[3] + gi_T[5]) - (gi_T[2] + gi_T[0]));
  Kc_s[12] = exp((gi_T[4] + gi_T[0]) - (gi_T[3] + gi_T[1]));
  Kc_s[13] = exp((gi_T[3] + gi_T[1]) - (gi_T[4] + gi_T[0]));
  Kc_s[14] = exp((gi_T[5] + gi_T[1]) - (gi_T[3] + gi_T[0]));
  Kc_s[15] = exp((gi_T[3] + gi_T[0]) - (gi_T[5] + gi_T[1]));
...
void speciesInternalEnergy(double *species, const double *tc) {
  ...
  if (T < 1000) {
    ...
    species[5] = OH_lo[0] - 1.0 + OH_lo[1] / 2.0 * tc[1] +
                 ...
                 OH_lo[4] / 5.0 * tc[4];
    /*species 8: N2 */
    species[6] = N2_lo[0] - 1.0 + N2_lo[1] / 2.0 * tc[1] +
...
/* SI unit*/
/* J/m^3*/
void CKUBMS(double T, const double *rhoi, double &ubms) {
  double result = 0;
...
/*tc contains precomputed powers of T, tc[0] = log(T) */
void cp_R(double *species, const double *tc) {
...
/*J/(m^3 K)*/
void CKCPBS(double T, const double *rhoi, double &cvbs) {
  double result = 0;
  double tc[] = {0, T, T * T, T * T * T, T * T * T * T}; /*temperature cache */
...
/*J/(m^3 K)*/
void CKCVBS(double T, const double *rhoi, double &cvbs) {
  double result = 0;
  double tc[] = {0, T, T * T, T * T * T, T * T * T * T}; /*temperature cache */
  double cpor[NSPECS];                                   /* temporary storage */
  cp_R(cpor, tc);
...
/*J/(m^3 K)*/
void CKGAMMA(double T, const double *rhoi, double &gamma) {
  double cp = 0, cv = 0;
  double tc[] = {0, T, T * T, T * T * T, T * T * T * T}; /*temperature cache */
  double cpor[NSPECS];                                   /* temporary storage */
  cp_R(cpor, tc);
...
void gauss(double *x, double (&A)[NSPECS][NSPECS], const double *b) {
  int i, j, k;
