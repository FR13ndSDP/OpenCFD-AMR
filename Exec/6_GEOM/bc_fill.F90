...
contains
  ...
  subroutine nc_hypfill(adv,adv_lo,adv_hi,domlo,domhi,delta,xlo,time,bc) &
    ...
    if (bc(2,1,1).eq.amrex_bc_ext_dir) then
      do     k = adv_lo(3), adv_hi(3)
    ...
    if (bc(3,1,1).eq.amrex_bc_ext_dir) then
    ...
    if (bc(3,2,1).eq.amrex_bc_ext_dir) then
      do     k = adv_hi(3), domhi(3)
        do   j = adv_lo(2), adv_hi(2)
          do i = adv_lo(1), adv_hi(1)
            adv(i,j,k,1) = rho_l
            adv(i,j,k,2) = rho_l * u_l
